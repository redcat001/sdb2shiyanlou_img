...
if [ ! -d "/opt/sequoiasql/mysql/database/3306/" ];then
   ...
   gosu sdbadmin /opt/sequoiasql/mysql/bin/sdb_sql_ctl restart  myinst
fi
...
if [ ! -d "/opt/sequoiasql/postgresql/database/5432/" ];then
  ...
  gosu sdbadmin /opt/sequoiasql/postgresql/bin/sdb_sql_ctl restart myinst
fi
...
gosu sdbadmin /opt/spark/sbin/stop-all.sh
gosu sdbadmin /opt/spark/sbin/stop-thriftserver.sh
...
gosu sdbadmin /opt/sequoiadb/bin/sdb -f /add_2copy.js 
